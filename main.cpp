...
int main(){
    Character catatau, catabol, gripen, yu, nobili, lopes;
    struct Potion verri = {
        "Verri", {Potion_Type::no_key, Potion_Type::friday_no_class}, {5, 5}};
    struct Potion fritz_rene = {
        "Fritz_Rene", {Potion_Type::renefritz_1potion, Potion_Type::renefritz_2potion}, {5, 5}};
    ...
    load_character(stage, &catatau);
    load_character(stage, &catabol);
    load_character(stage, &gripen);
    load_character(stage, &yu);
    load_character(stage, &nobili);
    load_character(stage, &lopes);
    ...
    if(lopes.status != Status::Tired){
        fight(lopes, catatau, catabol, gripen, professor_turn);
    }
    return 0;
}
